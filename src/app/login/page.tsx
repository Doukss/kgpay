    ...
    <div className="max-w-lg mx-auto p-6">
      ...
      {agencies.length === 0 ? (
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-slate-600">Aucune agence disponible. Commencez par créer une agence.</p>
          <a href="/agencies/signup" className="inline-flex mt-4 rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">
            Créer une agence
          </a>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <button type="submit" className="inline-flex items-center justify-center rounded-xl bg-brand-primary px-4 py-3 text-sm font-semibold text-white hover:bg-brand-primary-light">
            Se connecter
          </button>
        </form>
      )}
