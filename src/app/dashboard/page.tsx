...
export default function DashboardOverview() {
  ...
  const collectedRevenue = tenants.reduce((acc, t) => {
    ...
    const thisMonthPayments = t.payments.reduce(
      (pAcc, p) => pAcc + p.amount,
      0,
    );
  ...
  const recoveryRate =
    expectedRevenue > 0 ? (collectedRevenue / expectedRevenue) * 100 : 0;

  const overdueTenantsCount = tenants.filter(
    (t) => t.status === "overdue",
  ).length;
  const paidTenantsCount = tenants.filter((t) => t.status === "paid").length;
  const remindedTenantsCount = tenants.filter(
    (t) => t.status === "reminded",
  ).length;
  const unpaidTenantsCount = tenants.filter(
    (t) => t.status === "unpaid",
  ).length;
      ...
      <div className="rounded-2xl bg-slate-900 p-6 md:p-8 text-white border border-slate-800 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-xl">
          ...
          <p className="mt-2 text-sm text-slate-300 leading-relaxed">
            Suivez en temps réel les loyers de vos locataires, envoyez des
            relances Wave/WhatsApp en 1 clic et recevez les paiements
            instantanément.
          </p>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm flex items-start justify-between">
          <div className="space-y-2">
            ...
            <h3 className="text-2xl font-bold text-slate-900">
              {expectedRevenue.toLocaleString("fr-FR")}{" "}
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm flex items-start justify-between">
          <div className="space-y-2">
            ...
            <h3 className="text-2xl font-bold text-emerald-600">
              {collectedRevenue.toLocaleString("fr-FR")}{" "}
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm flex items-start justify-between">
          <div className="space-y-2">
            ...
            <h3 className="text-2xl font-bold text-slate-900">
              {outstandingRevenue.toLocaleString("fr-FR")}{" "}
              <span className="text-sm font-semibold text-slate-500">FCFA</span>
            </h3>
            <span
              className={`inline-flex items-center gap-1 text-[11px] font-bold ${outstandingRevenue > 0 ? "text-amber-600" : "text-emerald-600"}`}
            >
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm flex items-start justify-between">
          <div className="space-y-2">
            ...
            <h3
              className={`text-2xl font-bold ${overdueTenantsCount > 0 ? "text-rose-600" : "text-slate-900"}`}
            >
              {overdueTenantsCount} locataire
              {overdueTenantsCount > 1 ? "s" : ""}
            </h3>
            <span
              className={`inline-flex items-center gap-1 text-[11px] font-bold ${overdueTenantsCount > 0 ? "text-rose-600" : "text-slate-500"}`}
            >
              <AlertTriangle className="h-3.5 w-3.5" />
              <span>
                {overdueTenantsCount > 0
                  ? "Nécessite relance urgente"
                  : "Zéro retard critique"}
              </span>
            </span>
          </div>
          <div
            className={`rounded-xl p-3 border ${overdueTenantsCount > 0 ? "bg-rose-50 text-rose-600 border-rose-100" : "bg-slate-50 text-slate-600 border-slate-100"}`}
          >
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-slate-900">
                Historique de Collecte Mensuelle
              </h3>
              <p className="text-xs text-slate-400">
                Comparaison de collecte sur les 5 derniers mois
              </p>
            </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm space-y-6">
          <div>
            ...
            <p className="text-xs text-slate-400">
              État de paiement des locataires actifs
            </p>
          </div>
          ...
          <div className="h-4 w-full bg-slate-100 rounded-full overflow-hidden flex shadow-inner">
            <div
              style={{
                width: `${totalTenants > 0 ? (paidTenantsCount / totalTenants) * 100 : 0}%`,
              }}
            ...
            <div
              style={{
                width: `${totalTenants > 0 ? (remindedTenantsCount / totalTenants) * 100 : 0}%`,
              }}
            ...
            <div
              style={{
                width: `${totalTenants > 0 ? (unpaidTenantsCount / totalTenants) * 100 : 0}%`,
              }}
            ...
            <div
              style={{
                width: `${totalTenants > 0 ? (overdueTenantsCount / totalTenants) * 100 : 0}%`,
              }}
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs font-semibold">
              ...
              <span className="text-slate-950">
                {paidTenantsCount} (
                {totalTenants > 0
                  ? Math.round((paidTenantsCount / totalTenants) * 100)
                  : 0}
                %)
              </span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs font-semibold">
              ...
              <span className="text-slate-950">
                {remindedTenantsCount} (
                {totalTenants > 0
                  ? Math.round((remindedTenantsCount / totalTenants) * 100)
                  : 0}
                %)
              </span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs font-semibold">
              ...
              <span className="text-slate-950">
                {unpaidTenantsCount} (
                {totalTenants > 0
                  ? Math.round((unpaidTenantsCount / totalTenants) * 100)
                  : 0}
                %)
              </span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs font-semibold">
              ...
              <span className="text-slate-950">
                {overdueTenantsCount} (
                {totalTenants > 0
                  ? Math.round((overdueTenantsCount / totalTenants) * 100)
                  : 0}
                %)
              </span>
            </div>
          ...
          <div className="rounded-xl bg-slate-50 p-4 border border-slate-100 text-[11px] text-slate-500 leading-normal">
            Le statut <strong>Relancé</strong> passe automatiquement à{" "}
            <strong>Payé</strong> dès que le locataire effectue le paiement
            depuis son lien reçu.
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between pb-4 border-b border-slate-50">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Historique des derniers règlements enregistrés
            </p>
          </div>
          <Link
            href="/dashboard/recovery"
        ...
        {allPayments.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 text-slate-400">
            ...
            <span className="text-[10px] text-slate-500 mt-1">
              Utilisez le portail de paiement pour simuler une rentrée
              d&apos;argent.
            </span>
          </div>
        ) : (
