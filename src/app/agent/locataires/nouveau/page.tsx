...
const tenantFormSchema = z.object({
  ...
  telephone: z
    ...
    .regex(
      /^\+2217[0678]\d{7}$/,
      "Le téléphone doit être au format +221 7X XXX XX XX (ex: +221776543210)",
    ),
  operateurPrefere: z.enum(["wave", "orange_money", "especes", "virement"]),
  langue: z.enum(["francais", "wolof"]),
  bienDesignation: z
    .string()
    .min(3, "La désignation du bien est requise (ex: Appart. 4B)"),
...
export default function NouveauLocatairePage() {
  ...
  const currentTenantsCount = useKeurGuiStore(
    (state) =>
      state.tenants.filter((t) => t.agenceId === activeAgencyId).length,
  );
  const quotaReached = agency
    ? currentTenantsCount >= agency.quotaLocataires
    : false;
  ...
  const onSubmit = (data: TenantFormData) => {
    if (quotaReached) {
      toast.error(
        "Impossible d'ajouter un locataire : votre quota d'abonnement est dépassé.",
      );
      return;
    }
    ...
    <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-red-50 border border-red-200 text-red-800 rounded-xl p-4 flex gap-3 items-start animate-shake">
          ...
          <div>
            ...
            <p className="text-xs text-red-700 mt-1">
              Votre plan actuel ({agency?.plan.toUpperCase()}) limite votre
              agence à {agency?.quotaLocataires} locataires. Vous ne pouvez pas
              ajouter de nouveau locataire sans mettre à niveau votre plan dans
              les paramètres.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex justify-between items-center select-none">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs font-bold ${step >= 1 ? "text-slate-800" : "text-slate-400"}`}
          >
            Locataire
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs font-bold ${step >= 2 ? "text-slate-800" : "text-slate-400"}`}
          >
            Bail & Loyer
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs font-bold ${step >= 3 ? "text-slate-800" : "text-slate-400"}`}
          >
            Confirmation
          </span>
        </div>
                ...
                <div>
                  ...
                  <div className="space-y-2.5">
                    {[
                      ...
                      <label
                        key={op.id}
                        className="flex items-center gap-3 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 cursor-pointer text-sm"
                      >
                        <input
                          type="radio"
                          value={op.id}
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {errors.bienDesignation && (
                    <p className="text-xs text-red-600 mt-1 font-medium">
                      {errors.bienDesignation.message}
                    </p>
                  )}
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                    Montant caution (Optionnel, FCFA)
                  </label>
                  <input
                    type="number"
                ...
                <div className="flex items-center justify-between p-3 rounded-lg border border-slate-250 bg-slate-50">
                  <div className="space-y-0.5">
                    ...
                    <span className="text-[10px] text-slate-400 block">
                      Le locataire peut verser des acomptes.
                    </span>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-50 rounded-xl p-5 border border-slate-150 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500">
                      {formatPhone(formValues.telephone)}
                    </p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] uppercase font-bold text-slate-400 block">
                    ...
                    <p className="text-sm text-slate-700">
                      ...
                      <span className="font-bold capitalize">
                        {formValues.operateurPrefere.replace("_", " ")}
                      </span>
                    </p>
                    <p className="text-xs text-slate-500">
                      Relances en{" "}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-bold text-slate-850">
                      {formValues.bienDesignation}
                    </p>
                    <p className="text-xs text-slate-500">
                      {formValues.quartier}, Dakar
                    </p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] uppercase font-bold text-slate-400 block">
                    ...
                    <p className="text-xs text-slate-500">
                      Dû le {Number(formValues.jourEcheance) || 0} de chaque
                      mois
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="font-semibold text-slate-700">
                      Caution:{" "}
                      {formValues.montantCaution
                        ? formatFCFA(Number(formValues.montantCaution))
                        : "Aucune"}
                    </p>
                    <p className="text-slate-500">
                      Paiements partiels :{" "}
                      <span className="font-bold">
                        {formValues.paiementPartielAutorise
                          ? "Autorisés ✓"
                          : "Interdits ✗"}
                      </span>
                    </p>
              ...
              <div className="rounded-xl border border-slate-250 p-4 flex gap-3 bg-blue-50/50">
                ...
                <p className="text-xs text-blue-800 leading-normal">
                  En cliquant sur Enregistrer, le locataire sera ajouté au
                  tableau de bord. Il recevra un SMS/WhatsApp automatique de
                  relance **3 jours avant** le jour{" "}
                  {Number(formValues.jourEcheance) || 0} du mois.
                </p>
              </div>
            </div>
          )}
