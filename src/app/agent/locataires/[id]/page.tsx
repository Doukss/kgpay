...
export default function LocataireFichePage() {
  ...
  const [payMethod, setPayMethod] = useState<
    "wave" | "orange_money" | "especes" | "virement"
  >("wave");
  ...
  const tenant = tenants.find(
    (t) => t.id === tenantId && t.agenceId === activeAgencyId,
  );
  ...
  const tenantPayments = payments
    .filter((p) => p.locataireId === tenantId)
    .sort((a, b) => b.mois.localeCompare(a.mois));
  const tenantReminders = reminders
    .filter((r) => r.locataireId === tenantId)
  ...
  const pctPaid = Math.min(
    100,
    Math.round((tenant.montantPayeMoisCourant / tenant.loyerMensuel) * 100),
  );
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div className="flex items-center gap-4">
          <div className="h-16 w-16 rounded-2xl bg-blue-100 border border-blue-200 text-blue-700 flex items-center justify-center font-black text-xl shrink-0">
            {tenant.prenom.slice(0, 1)}
            {tenant.nom.slice(0, 1)}
          </div>
          <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 text-sm">
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-slate-850">
                      {formatFCFA(tenant.loyerMensuel)} / mois
                    </span>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-700">
                      Chaque {tenant.jourEcheance} du mois
                    </span>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-700">
                      {new Date(tenant.dateDebutBail).toLocaleDateString(
                        "fr-FR",
                      )}
                    </span>
                  </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-700 capitalize">
                      {tenant.operateurPrefere.replace("_", " ")}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-700">
                      {tenant.montantCaution
                        ? formatFCFA(tenant.montantCaution)
                        : "Aucune"}
                    </span>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-baseline text-sm">
                    ...
                    <span className="font-black text-slate-800">
                      {formatFCFA(tenant.montantPayeMoisCourant)} /{" "}
                  ...
                  <div className="text-[10px] text-right text-slate-400 font-bold">
                    {pctPaid}% complété
                  </div>
                </div>
                ...
                <div className="border-t border-slate-100 pt-3 flex justify-between items-center text-xs">
                  ...
                  <CountdownEcheance
                    statut={tenant.statutMoisCourant}
                    jourEcheance={tenant.jourEcheance}
                  />
                </div>
              </div>
              ...
              {tenant.arrieresCumules > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-5 shadow-xs space-y-2">
                  ...
                  <h4 className="text-2xl font-black text-red-600 tracking-tight">
                    {formatFCFA(tenant.arrieresCumules)}
                  </h4>
                  <p className="text-[11px] text-red-700 leading-normal">
                    Ce montant correspond à{" "}
                    {Math.round(tenant.arrieresCumules / tenant.loyerMensuel)}{" "}
                    loyer(s) mensuel(s) impayé(s) des mois précédents.
                  </p>
                </div>
              )}
            </div>
                  ...
                  <tbody className="divide-y divide-slate-150">
                    {tenantPayments.map((p) => {
                      ...
                      return (
                        <tr
                          key={p.id}
                          className={`hover:bg-slate-50/30 transition-colors ${textClass}`}
                        >
                          <td className="px-6 py-4 font-bold capitalize">
                            {new Date(p.mois + "-02").toLocaleDateString(
                              "fr-FR",
                          ...
                          <td className="px-6 py-4 text-right">
                            {isComplete ? (
                              <button
                                onClick={() =>
                                  toast.success(
                                    "Reçu PDF en cours de téléchargement (Simulé)",
                                  )
                                }
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between gap-4">
              <div>
                <h4 className="font-bold text-slate-800 text-sm">
                  Déclencher une relance manuelle immédiate
                </h4>
                <p className="text-xs text-slate-500 mt-0.5">
                  Envoyer un SMS ou un message WhatsApp personnalisé.
                </p>
              </div>
              <button
                onClick={() => setIsReminderModalOpen(true)}
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
              ...
              {tenantReminders.length === 0 ? (
                <div className="text-center text-slate-400 py-6">
                  <Inbox className="h-8 w-8 mb-2 mx-auto" />
                  <p className="text-sm">
                    Aucune relance n'a encore été envoyée à ce locataire.
                  </p>
                </div>
              ) : (
                  ...
                  {tenantReminders.map((rel) => {
                    ...
                    return (
                      <div key={rel.id} className="relative">
                        {/* Bullet point icon */}
                        <span
                          className={`absolute -left-[31px] top-1 h-5 w-5 rounded-full border border-white flex items-center justify-center ${
                            isSuccess
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-extrabold uppercase text-slate-700 tracking-wider">
                              Relance {rel.type.toUpperCase()} •{" "}
                              {rel.canal.toUpperCase()}
                            </span>
                            <span className="text-[10px] text-slate-400">
                              {new Date(rel.dateEnvoi).toLocaleDateString(
                                "fr-FR",
                              )}{" "}
                              à{" "}
                              {new Date(rel.dateEnvoi).toLocaleTimeString(
                                "fr-FR",
                          ...
                          {!isSuccess && (
                            <div className="text-[10px] text-red-600 font-bold flex items-center gap-1">
                              <span>
                                Échec d'envoi (Numéro ou quota invalide)
                              </span>
                            </div>
                          )}
                        </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-xl max-w-md w-full overflow-hidden animate-slide-in">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="rounded-lg bg-orange-50 border border-orange-200 p-3 text-xs text-orange-800 leading-normal flex gap-2">
                ...
                <p>
                  Ce message sera envoyé directement au numéro{" "}
                  <strong>{formatPhone(tenant.telephone)}</strong>. Vous pourrez
                  suivre son statut dans l'onglet des relances.
                </p>
              </div>
            </div>
